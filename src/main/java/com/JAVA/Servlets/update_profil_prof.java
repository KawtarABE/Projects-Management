...
public class update_profil_prof extends HttpServlet {
	private static final long serialVersionUID = 1L;

    public update_profil_prof() {
        super();
    }
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		obj.updateProf(nom, prenom, domaine, departement,id_user);
		obj.updateUser(id_user, nom, prenom, email, numero);
		request.setAttribute("success", "Your profil was successfuly updated");
		request.getRequestDispatcher("profil_prof").forward( request, response );
	}
