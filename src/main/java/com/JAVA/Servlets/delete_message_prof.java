...
public class delete_message_prof extends HttpServlet {
	private static final long serialVersionUID = 1L;

    public delete_message_prof() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if( action.equals("send")) {
        	request.getRequestDispatcher("sent_mail_prof").forward( request, response );
        }
        
        else {
        	request.getRequestDispatcher("mail_prof").forward( request, response );
        }
	}
